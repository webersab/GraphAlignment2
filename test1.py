...
def test(sentence_to_relations_dict,lambda_value,graphs_file_path,  output_file_path, language_flag):
    ...
    sentence_counter=0
    #hack, remove later
    #new_sentence_to_relations_dict={}
    for sentence in sentence_to_relations_dict:
        ...
        for relation in relation_dict["rel1"]:
            types_set.add(relation[1])
            app_rel=relation[0]
            if relation[0]=='is.1,is' or relation[0]=='is.an.1,is.an' or relation[0]=='is.am.1,is.am' or relation[0]=='is.in.1,is.in':
                app_rel="sein.1,sein"
            relations_list1.append(app_rel)
        for relation in relation_dict["rel2"]:
            types_set.add(relation[1])
            app_rel=relation[0]
            if relation[0]=='is.1,is' or relation[0]=='is.an.1,is.an' or relation[0]=='is.am.1,is.am' or relation[0]=='is.in.1,is.in':
                app_rel="sein.1,sein"
            relations_list2.append(app_rel)
        ...
        #else:
            #hack, remove later
            #new_sentence_to_relations_dict[sentence]=relation_dict
        if judgement=="y":
            if "y" in our_judgement_set:
                tp+=1
                #print("-----------------True positive-----------------")
        ...
        total+=1
    #hack, remove later
    #pickle.dump(new_sentence_to_relations_dict,open("relation_dict_minimal.pickle","wb"))
    #fill test_dict object with stats
    print("tp ",tp, " tn ",tn," fn ",fn," fp ", fp)
    print("\n precision: "+str(tp/(tp+fp)))
    print("\n recall: "+str(tp/(tp+fn)))
...
if __name__ == '__main__':
    ...
    #lam=0.87
    sentence_to_relation_dict = pickle.load( open( "relation_dict_minimal.pickle", "rb" ) )
    test_dict=test(sentence_to_relation_dict,lam,"multilingual_graphs/","testResults/multiMinimal/","multi")
